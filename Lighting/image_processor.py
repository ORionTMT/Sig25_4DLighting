...
def estimate_scene_depth(image, depth_estimator):
    ...
    #depth_map = depth_map*0.8+0.2
    image = torch.cat([depth_map] * 3, dim=1)

    image = image.permute(0, 2, 3, 1).cpu().numpy()[0]
    array_image = (image * 255.0).clip(0, 255).astype(np.uint8)
    image = Image.fromarray((image * 255.0).clip(0, 255).astype(np.uint8))
    return image

def fill_depth_circular(depth_image, x, y, r, depth=None, depth_mode="nearest", mask=None):
    depth_image = np.array(depth_image)
    if depth_mode == "nearest":
        depth_value = 255
    elif depth_mode == "pixel":
        depth_value = depth_image[y + r // 2, x + r // 2, 0]
    elif depth_mode == "local_nearest":
        depth_value = depth_image[y:y+r, x:x+r].max()
    elif depth_mode == "local_far":
        depth_value = depth_image[y:y+r, x:x+r].min()
    elif depth_mode == "local":
        depth_value = (np.power(float(1.0)/depth, 1/2.3)) * 255
    elif depth_mode == 'farthest':
        depth_value = 5
    elif depth_mode == "midway":
        depth_value = (depth_image[y + r // 2, x + r // 2, 0].item() + 255.0) / 2
