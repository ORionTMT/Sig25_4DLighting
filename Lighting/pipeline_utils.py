...
def custom_prepare_latents(
        ...
        if latents is None and use_noise_moving:
            ...
            newrs = [newr] if not isinstance(newr, list) else newr
            big_noise = noise.clone()
            prev_noise = None
            for newy, newx, newr in zip(newys, newxs, newrs):
                ...
                if prev_noise is not None:
                    ...
                    sy_start = int(sy)
                    sy_end = int(sy + shape[2])
                    sx_start = int(sx)
                    sx_end = int(sx + shape[3])

                    # Use the integer indices
                    noise = big_noise[:, :, sy_start:sy_end, sx_start:sx_end]
...
def custom_prepare_mask_latents(
    ...
    # we do that before converting to dtype to avoid breaking in ca;'//se we're using cpu_offload
    # and half precision
